...
window.onload = function() {
    ...
    var queue = {
        ...
        paint: function() {
            var str = "";
            // var str = this.str.map(function(item){return ("<div>" + parseInt(item) + "</div>");});

            function(item){str += ("<div>" + parseInt(item) + "</div>")};
    ...
    function addDivDelEvent() {
        for (var cur = 0; cur < container.childNodes.length; cur++) {
            ...
            addEvent(container.childNodes[cur], "click", function(cur1) {
                return function(){return queue.deleteID(cur1)};
            }(cur));
        }
    }
    ...
    //     }
    // });
    addEvent(buttonList[1], "click", function(){queue.leftPush()});
